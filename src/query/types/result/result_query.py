...
class QueryDataCollection:
    def __init__(self, core: BaseCore, data_set: dict):
    ...
    def flatten_totient(self):
        ...
        for data_int in self.data_set.keys():
            ...
            totient_ls = []
            while tot_elem is not None and self.__core.is_greater(tot_elem.value, self.__core.m_identity):
                tot_val = tot_elem.value
                tot_elem = tot_elem.totient_element
                totient_ls.append(tot_val)
            totient_ls.append(self.__core.m_identity)
